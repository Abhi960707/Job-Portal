...
import { Contact, Mail, Pen, FileText, Building2, Briefcase } from 'lucide-react'
        ...
        <div className="bg-gray-50 min-h-screen pb-10">
            ...
            <div className='max-w-4xl mx-auto mt-6 px-4'>
                <BackButton
                    ...
                    label={isRecruiter ? "Back to Dashboard" : "Back to Home"}
                />
            </div>

            {/* Profile Card */}
            <div className='max-w-4xl mx-auto bg-white border border-gray-100 rounded-2xl shadow-sm mt-4 p-8'>
                <div className='flex justify-between items-start'>
                    <div className='flex items-center gap-6'>
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                ...
                                <Badge className={`border-none capitalize font-semibold ${
                                    isRecruiter
                            ...
                            <p className="text-gray-600 mt-2 max-w-md">
                                {user?.profile?.bio || (isRecruiter
                                    ? "Recruiter — building great teams."
                                    : "Passionate professional looking for new opportunities. Add a bio to tell recruiters more about yourself."
                                )}
                            </p>
                        </div>
                    </div>
                    <Button
                        ...
                        className='border-[#6A38C2] text-[#6A38C2] hover:bg-[#6A38C2] hover:text-white transition-colors'
                        variant='outline'
                    >
                ...
                {isRecruiter && (
                    <div className="mt-8 pt-8 border-t border-gray-100">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <Button onClick={() => navigate("/admin/companies")} variant="outline" className="flex items-center gap-2 border-[#6A38C2] text-[#6A38C2] hover:bg-purple-50">
                                <Building2 className="h-4 w-4" /> My Companies
                            </Button>
                            <Button onClick={() => navigate("/admin/jobs")} variant="outline" className="flex items-center gap-2 border-blue-400 text-blue-600 hover:bg-blue-50">
                                <Briefcase className="h-4 w-4" /> Posted Jobs
                            </Button>
                            <Button onClick={() => navigate("/admin/applicants")} className="flex items-center gap-2 bg-[#6A38C2] hover:bg-[#5d07f1] text-white">
                                <Mail className="h-4 w-4" /> All Applicants
                            </Button>
                        </div>
                    </div>
                )}
